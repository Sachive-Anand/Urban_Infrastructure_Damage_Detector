...
export default function App() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/scan?conf=${conf / 100}`, {
        method: 'POST',
        body: fd,
      })
      ...
      setError(
        e.message.includes('fetch')
          ? 'Cannot reach backend. Server may be waking up — wait 30s and try again.'
          : e.message
      )
    } finally {
      setLoading(false)
  ...
  // ── Remove scan from S3 + reset UI ──
  // Sends DELETE request with scan_id + both S3 keys
  // Backend deletes both input and output files from S3
  // Then resets UI so user can upload a new image
  const removeScan = async () => {
    if (!result) return
    setRemoving(true)
    try {
      const date  = result.timestamp.split('T')[0]
      const input_key  = `inputs/${date}/${result.scan_id}.jpg`
      const output_key = `outputs/${date}/${result.scan_id}_annotated.png`

      await fetch(`${API}/scan/${result.scan_id}`, {
        method: 'DELETE',
      ...
      // Whether delete succeeded or failed — reset UI
      // User should always be able to re-upload
      reset()
    } catch (e) {
      // Even on error — let user re-upload
      reset()
    } finally {
      setRemoving(false)
    }
      ...
      <header style={css.header}>
        <div style={css.headerInner}>
          ...
          <div style={css.headerBadges}>
            ...
            <div style={{ ...css.badge, color: 'var(--muted2)' }}>
              YOLOv8 · 8 classes
            </div>
            <div style={{ ...css.badge, color: 'var(--muted2)' }}>
              ☁ AWS S3
        ...
        <aside style={css.sidebar}>
          ...
          <div style={css.block}>
            ...
            <div
              style={{
                ...css.dropzone,
                ...(drag ? css.dropActive : {}),
                ...(preview ? { cursor: 'default', minHeight: 'auto' } : {}),
              }}
            ...
            >
              ...
              {preview ? (
                <div style={{ position: 'relative' }}>
                  ...
                  {/* Only show Change button if no result yet */}
                  {!result && (
                    <button
                      style={css.changeBtn}
          ...
          <div style={css.block}>
            <div style={{ ...css.blockLabel, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: 'var(--accent)', fontFamily: 'var(--font-head)', fontSize: 13 }}>
                {conf}%
              </span>
            </div>
            <input
              type="range" min="5" max="95" value={conf}
              onChange={e => setConf(+e.target.value)}
              style={css.slider}
            />
            <div style={css.sliderHints}>
              <span>More detections</span>
              <span>Fewer, surer</span>
            </div>
          </div>

          {/* Detect button — hidden once result exists */}
          {!result && (
            <button
          ...
          {/* Remove & Re-upload — shown only after result */}
          {result && (
            <button
              style={{ ...css.removeBtn, ...(removing ? css.analyzeDim : {}) }}
              onClick={removeScan}
              disabled={removing}
            >
              {removing
                ? <><span style={{ ...css.spin, borderTopColor: '#fff' }} />Removing...</>
                : <>🗑 Remove & Re-upload</>
              }
            </button>
          )}

          {/* Clear — shown while uploading before result */}
          {file && !result && !loading && (
          ...
          {/* Scan info — shown after result */}
          {result && (
            <div style={css.scanInfo}>
              <div style={css.blockLabel}>SCAN INFO</div>
              <div style={css.scanInfoRow}>
                <span style={css.scanInfoLabel}>Scan ID</span>
                <span style={css.scanInfoValue}>{result.scan_id.slice(0, 8)}...</span>
              </div>
              <div style={css.scanInfoRow}>
                <span style={css.scanInfoLabel}>Timestamp</span>
                <span style={css.scanInfoValue}>
                  {new Date(result.timestamp).toLocaleTimeString()}
                </span>
              </div>
              <div style={css.scanInfoRow}>
                <span style={css.scanInfoLabel}>Storage</span>
                <span style={{ ...css.scanInfoValue, color: '#22c55e' }}>☁ Saved to S3</span>
              </div>
              <div style={css.scanInfoRow}>
                <span style={css.scanInfoLabel}>Image size</span>
                <span style={css.scanInfoValue}>
            ...
            {result && (
              <div style={css.tabs}>
                {['annotated', 'original'].map(t => (
                  <button
                    key={t}
                    style={{ ...css.tabBtn, ...(tab === t ? css.tabActive : {}) }}
                    onClick={() => setTab(t)}
                  >
            ...
            <div style={css.imageBox}>
              ...
              {loading && (
                <div style={css.loadOverlay}>
                  <div style={css.loadBox}>
                    ...
                    <div style={css.loadSub}>YOLOv8 scanning · Saving to S3...</div>
                    <div style={css.loadBar}>
                      <div style={css.loadBarFill} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {removing && (
                <div style={css.loadOverlay}>
                  <div style={css.loadBox}>
                    <div style={css.bigSpin} />
                    <div style={css.loadTitle}>Removing from S3</div>
                    <div style={css.loadSub}>Deleting scan files...</div>
                  </div>
                </div>
              )}

              {/* Annotated image from S3 URL */}
              {result && tab === 'annotated' && !loading && !removing && (
                <img
              ...
              {/* Original image from S3 URL */}
              {result && tab === 'original' && !loading && !removing && (
                <img
...
const css = {
  ...
  dropTitle: {
    ...
    color: 'var(--muted2)', marginBottom: 5,
  },
  ...
  removeBtn: {
    ...
    color: '#fff', fontFamily: 'var(--font-head)', fontSize: 14, fontWeight: 800,
    cursor: 'pointer', letterSpacing: '0.03em',
  ...
  scanInfoRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  scanInfoLabel: { fontSize: 11, color: 'var(--muted)' },
  scanInfoValue: { fontSize: 11, color: 'var(--muted2)', fontFamily: 'monospace' },
  ...
  statLbl: {
    ...
    letterSpacing: '0.08em', fontFamily: 'var(--font-head)',
  },
  imageCard: {
